...
num_particles = 300;
distance = 100;
speed = 0.75; //$("#speed").val() / 100;
radius = 2;
lineWidth = 100 / 100;
particleColor = "#3434ff";
lineColor = "#3434ff";
...
speed = 0.5; //$("#speed").val() / 100;
radius = $("#radius").val();
lineWidth = $("#lineWidth").val() / 100;
particleColor = "#" + $("#particleColor").val();
...
// ========== Controls Menu ========== \\
toggle = 1;
$("#ctrl_toggle").on("click", function() {
    if (toggle == 1) {
        $("#ctrl_menu").animate({ height: 20 + "px" });
        toggle = 0;
    } else {
        $("#ctrl_menu").animate({ height: 300 + "px" });
        toggle = 1;
    }
});

$("#num_particles").on("change", function() {
    num_particles = $(this).val();
});
...
$("#lineWidth").on("change", function() {
    lineWidth = $(this).val() / 100;
});
$("#particleColor").on("change", function() {
    particleColor = "#" + $(this).val();
...
Particle.prototype.Update = function() {
    ...
    if (this.x < 0 + radius || this.x + radius > canvas.width)
        this.vx = -this.vx;
